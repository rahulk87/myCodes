...
setwd('./')
results      = read.csv("tnbc_atm_4926_results_for_fig.csv", header=T, stringsAsFactors = F, sep="\t")
...
pdf("Mut_type_ggplot.pdf", width=12, height=120)
...
p <- ggplot(df33, aes(x=samples,y=genes,fill=mut_type,group=loh)) + geom_tile(width=0.8,height=0.8) + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=1) + theme(axis.text.x=element_text(size=20,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=10, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))

dev.off()

#################################################
## Plot for genes with more than two mutations ##
#################################################
library(data.table)

df11 = data.table(df11)

df11 <- df11[, `freq` := .N, by = genes]

df11_mt2 <- subset(df11, freq >= 2)

df11_mt2 <- df11_mt2[order(df11_mt2$samples, df11_mt2$types, decreasing=TRUE),]

genes_lt2 <- subset(df11, freq < 2)$genes

mutation_heatmap_red <- mutation_heatmap[!grepl(paste(genes_lt2, collapse="|"), rownames(mutation_heatmap)), ]

df11_mt2$genes <- factor(df11_mt2$genes, levels = dput(rev(rownames(mutation_heatmap_red))))

comb<-expand.grid(samples = unique(df11_mt2$samples), genes = unique(df11_mt2$genes), ccf=".", clone = ".", loh=".", mut_type="I", types = 9, freq = ".")

df33_mt2 <- as.data.frame(rbind(comb, df11_mt2))

df22_mt2<-subset(df11_mt2,clone=="Clonal")

pdf("Mut_type_ggplot_mt2.pdf", width=12, height=120)

p <- ggplot(df33_mt2, aes(x=samples,y=genes,fill=mut_type,group=loh)) + geom_tile(width=0.8,height=0.8) + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=1) + theme(axis.text.x=element_text(size=20,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=10, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
...
print (width)
print (height)
pdf("CCF_Heatmap_ggplot.pdf", width=12, height=120) ## Change size of the plot
...
p <- ggplot(df3, aes(x=samples,y=genes,fill=ccf_groupB, group=loh)) + geom_tile(width=0.8,height=0.8) + geom_tile(data=df2,aes(samples, genes),size=0.7,fill=NA,width=0.8,height=0.8,color="gold3") + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=1) + theme(axis.text.x=element_text(size=20,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=10, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
