...
setwd('./')
results      = read.csv("atcclr_azltr_input.txt", header=T, stringsAsFactors = F, sep="\t")
...
muts$newGene <- paste(muts$newGene, muts$AA, sep="_")
muts$newGene <- sub("_"," (",muts$newGene)
muts$newGene <- sub("$*",")", muts$newGene)
#########
...
smallmaf$newGene <- paste(smallmaf$CGC, smallmaf$SYMBOL, sep="")
smallmaf$newGene <- paste(smallmaf$newGene, smallmaf$AA, sep="_")
smallmaf$newGene <- sub("_"," (", smallmaf$newGene)
smallmaf$newGene <- sub("$*",")", smallmaf$newGene)
...
pdf("Mut_type_ggplot.pdf", width=1.4, height=15)

p <- ggplot(df33, aes(x=samples,y=genes,fill=mut_type,group=loh)) + geom_tile(width=0.9,height=0.9) + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=0.2) + theme(axis.text.x=element_text(size=5,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=5, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
...
print (width)
print (height)
pdf("CCF_Heatmap_ggplot.pdf", width=1.4, height=15) ## Change size of the plot

p <- ggplot(df3, aes(x=samples,y=genes,fill=ccf_groupB, group=loh)) + geom_tile(width=0.8,height=0.8) + geom_tile(data=df2,aes(samples, genes),size=0.5,fill=NA,width=0.8,height=0.8,color="gold3") + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=0.5) + theme(axis.text.x=element_text(size=5,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=5, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
