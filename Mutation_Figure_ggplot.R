...
setwd('./')
results      = read.csv("ovary_granulosa_batch2_for_fig.csv", header=T, stringsAsFactors = F, sep="\t")
...
plot_file     = paste('Mutation_Heatmap_cgene', patients[b], ".pdf", sep="")
plot_file_CCF = paste('CCF_Heatmap_cgene', patients[b], ".pdf", sep="")
########################################################
muts$CCF_group = "0"
...
#muts = subset(muts, subset = muts$Patient == "RG10")# & muts$TUMOR_SAMPLE != "IDC19")# | muts$TUMOR_SAMPLE == "AM3" | muts$TUMOR_SAMPLE == "AM4" | muts$TUMOR_SAMPLE == "AM5 - Primary_AME" | muts$TUMOR_SAMPLE == "AM6" | muts$TUMOR_SAMPLE == "AM7" | muts$TUMOR_SAMPLE == "AM32AME1")# & muts$TUMOR_SAMPLE != "AM8-Ips-Breast-Rec-Carc" & muts$TUMOR_SAMPLE != "AM5 - Axillary_Lymph_Node_Metastasis" & muts$TUMOR_SAMPLE != "AM46T2" & muts$TUMOR_SAMPLE != "AM5 - Primary_Carcarcinoma" & muts$TUMOR_SAMPLE != "AM8-LNE" & muts$TUMOR_SAMPLE != "AM8-LNM")
muts = subset(muts, subset = muts$ANN....EFFECT != "synonymous_variant" & muts$ANN....EFFECT != "splice_region_variant&synonymous_variant" & muts$ANN....EFFECT != "downstream_SYMBOL_variant" 
          & muts$ANN....EFFECT != "intron_variant"
& muts$ANN....EFFECT != "non_coding_exon_variant|synonymous_variant" & muts$ANN....EFFECT != "SYNONYMOUS_CODING"  & muts$ANN....EFFECT != "Silent" & muts$ANN....EFFECT != "synonymous_variant|synonymous_variant|synonymous_variant" & muts$ANN....EFFECT != "synonymous_variant|synonymous_variant")
...
#muts = subset(muts, subset = muts$lawrence=="TRUE" | muts$kandoth =="TRUE" | muts$cancer_gene_census == "TRUE" | muts$pathogenicity == 'likely_pathogenic')
#muts = subset (muts, subset = muts$pathogenicity == 'likely_pathogenic')

sample_names   =  as.list(sort(unique(muts$TUMOR_SAMPLE))) 
if(length(sample_names) == 1){ sample_names = c(sample_names, "Spacer")}
mutation_genes = unique(muts$SYMBOL)
...
#muts = muts[muts$SYMBOL %in% mutation_genes,]
###########################################################################
...
if (!TCGA) { smallmaf <- muts[which(muts$SYMBOL %in% unlist(mutation_genes) & muts$TUMOR_SAMPLE %in% unlist(sample_names)),]
}else { 
...
for (i in 1:nrow(smallmaf)) {
  ...
  if (smallmaf$HOTSPOT[i] == "TRUE") { type = 1; smallmaf$mut_type[i] = 'A'; smallmaf$types[i] = 1
  ...
  if (!TCGA) { 
    if (mutation_heatmap[which(rownames(mutation_heatmap)==smallmaf$TUMOR_SAMPLE[i],), which(colnames(mutation_heatmap)==smallmaf$SYMBOL[i])] > type) {
      mutation_heatmap[which(rownames(mutation_heatmap)==smallmaf$TUMOR_SAMPLE[i],), which(colnames(mutation_heatmap)==smallmaf$SYMBOL[i])] <- type
...
for (i in 1:nrow(smallmaf)) {
  ...
  if (CCFgroup_heatmap[which(rownames(CCFgroup_heatmap)==smallmaf$TUMOR_SAMPLE[i],), which(colnames(CCFgroup_heatmap)==smallmaf$SYMBOL[i])] < type) {
    CCFgroup_heatmap[which(rownames(CCFgroup_heatmap)==smallmaf$TUMOR_SAMPLE[i],), which(colnames(CCFgroup_heatmap)==smallmaf$SYMBOL[i])] <- type}
}
i=1
...
gene_order = rev(rownames(mutation_heatmap))#[length(rownames(mutation_heatmap)):1]

samp_order = colnames(mutation_heatmap)

df11 <-as.data.frame(cbind(smallmaf$TUMOR_SAMPLE, smallmaf$SYMBOL, smallmaf$Cancer_Cell_Fraction, smallmaf$Clonal_Status, smallmaf$loh, smallmaf$mut_type, smallmaf$types))

colnames(df11) <- c("samples", "genes", "ccf", "clone", "loh", "mut_type", "types")

df11 <- df11[order(df11$samples, df11$types, decreasing=TRUE),]

df11$samples <- factor(df11$samples, levels = dput(samp_order))

df11$genes <- factor(df11$genes, levels = dput(gene_order))
...
pdf("Mut_type_ggplot.pdf", width=3.5, height=4)

p <- ggplot(df33, aes(x=samples,y=genes,fill=mut_type,group=loh)) + geom_tile(width=0.9,height=0.9) + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=0.5) + theme(axis.text.x=element_text(size=12,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=10, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
...
df1$genes <- factor(df1$genes, levels = dput(gene_order))

df1$samples <- factor(df1$samples, levels = dput(samp_order))
...
print (width)
print (height)
pdf("CCF_Heatmap_ggplot.pdf", width=3.5, height=4) ## Change size of the plot

p <- ggplot(df3, aes(x=samples,y=genes,fill=ccf_groupB, group=loh)) + geom_tile(width=0.8,height=0.8) + geom_tile(data=df2,aes(samples, genes),size=0.7,fill=NA,width=0.8,height=0.8,color="gold3") + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=0.5) + theme(axis.text.x=element_text(size=12,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=10, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))
...
for (i in 1:nrow(smallmaf)) {
  ...
  if (LOH_heatmap[which(rownames(LOH_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(LOH_heatmap)==smallmaf$SYMBOL[i],)] < type) {
    LOH_heatmap[which(rownames(LOH_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(LOH_heatmap)==smallmaf$SYMBOL[i],)] <- type}
}


#######################
#Validation
#Validation_heatmap <- matrix(0, ncol=sum(unlist(lapply(gene_order, length))), nrow=sum(unlist(lapply(samp_order, length))))
#colnames(Validation_heatmap) <- gene_order
#rownames(Validation_heatmap) <- samp_order

#for (i in 1:nrow(smallmaf)) {
#        type = smallmaf$Validation_Status..50.reads[i]
#        if (type == "Validated") {type = 1
#        } else if (type == "Not Validated") {type = 2
#        } else { type = 0 }
#       print(paste(i,type,sep="_"))
#        
#        if (Validation_heatmap[which(rownames(Validation_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(Validation_heatmap)==smallmaf$SYMBOL[i],)] < type) {
#                Validation_heatmap[which(rownames(Validation_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(Validation_heatmap)==smallmaf$SYMBOL[i],)] <- type}
#        
#}
...
for (i in 1:nrow(smallmaf)) {
  ...
  if (Clonal_heatmap[which(rownames(Clonal_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(Clonal_heatmap)==smallmaf$SYMBOL[i],)] < type) {
    Clonal_heatmap[which(rownames(Clonal_heatmap)==smallmaf$TUMOR_SAMPLE[i]), which(colnames(Clonal_heatmap)==smallmaf$SYMBOL[i],)] <- type}
}
...
#Validation_heatmap = Validation_heatmap[nrow(Validation_heatmap):1,]
#for (i in 1:nrow(Validation_heatmap)){
#row_plot = row_pos[as.vector(Validation_heatmap[i,]==1)] 
#         points( row_plot, rep(col_pos[(length(samp_order)+1)-i],length(row_plot)), pch="/", col="green", cex = 3)      
#}
#Validation_heatmap = Validation_heatmap[nrow(Validation_heatmap):1,]
#for (i in 1:nrow(Validation_heatmap)){
#        row_plot = row_pos[as.vector(Validation_heatmap[i,]==2)] 
#        points( row_plot, rep(col_pos[(length(samp_order)+1)-i],length(row_plot)), pch="/", col="red", cex = 3)      
#}
