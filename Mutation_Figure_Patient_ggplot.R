...
muts                 = muts[order(muts$ANN....GENE, decreasing=F),]
muts                 = muts[!is.na(muts$Cancer_Cell_Fraction),]
...
df11 <-as.data.frame(cbind(smallmaf$TUMOR_SAMPLE, smallmaf$ANN....GENE, smallmaf$Cancer_Cell_Fraction, smallmaf$Clonal_Status, smallmaf$loh, smallmaf$mut_type, smallmaf$types))

colnames(df11) <- c("samples", "genes", "ccf", "clone", "loh", "mut_type", "types")

df11 <- df11[order(df11$samples, df11$types, decreasing=TRUE),]

df11 <- as.data.frame(lapply(df11, function(x) {sub("DL-moc-0", "", x)})) ## Modify samples names (if required)
...
p <- ggplot(df33, aes(x=samples,y=genes,fill=mut_type,group=loh)) + geom_tile(width=0.8,height=0.8) + scale_fill_manual(values = cols, name="CCF",guide = FALSE); p + geom_segment( aes(x=xmin,xend=xmax,y=ymin,yend=ymax), subset(ggplot_build(p)$data[[1]],group==2), inherit.aes=F,color="white",size=0.5) + theme(axis.text.x=element_text(size=20,  family = "sans", angle = 90, hjust=0, vjust=0)) + theme(axis.text.y=element_text(size=20, face="italic", family = "sans")) + scale_x_discrete(name="",position="top",drop=FALSE) + scale_y_discrete(name="",drop=FALSE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank()) + theme(legend.position="none") + theme(axis.ticks = element_blank()) + theme(axis.text=element_text(colour = "black"))

dev.off()

######## More than two ####
genes <- c("KEAP1", "TGFBR1", "TET1", "TERT", "TBX3", "SPOP", "SMO", "SF3B1", "PTPRT", "PTEN", "PRKAR1A", "PIK3C2G", "PDPK1", "PAK1", "NFE2L2", "MYOD1", "MRE11A", "MITF", "MEN1", "MED12", "MDM2", "LATS1", "KDM5A", "JAK2", "IL7R", "IDH2", "HGF", "GRIN2A", "GATA1", "FLT1", "FGFR1", "EZH2", "ETV1", "ESR1", "ERBB4", "DCUN1D1", "CTCF", "CRLF2", "CBL", "BTK", "BRIP1", "BMPR1A", "BLM", "BCL2", "YES1", "XPO1", "TP63", "TOP1", "TMPRSS2", "SUFU", "STAG2", "SOX2", "ROS1", "RET", "RASA1", "PTPRS", "PBRM1", "NTRK3", "NTRK2", "NSD1", "NOTCH3", "MYCN", "MET", "MAP3K13", "KIT", "KDM5C", "JAK1", "IGF1", "FAT1", "EPHB1", "EPHA5", "CARD11", "BCOR", "ATRX", "ASXL2", "ARID2", "APC", "AMER1", "KDM6A", "NTRK1", "PMS1", "HNF1A", "SHQ1", "PIK3R2", "EED", "MSH6", "U2AF1L5", "HRAS", "HIST1H1C", "CIC", "PHOX2B", "CCND1")
...
df1 <-as.data.frame(cbind(smallmaf$TUMOR_SAMPLE, smallmaf$ANN....GENE, smallmaf$Cancer_Cell_Fraction, smallmaf$Clonal_Status, smallmaf$loh, smallmaf$CCF_group, smallmaf$types))
...
df1 <- as.data.frame(lapply(df1, function(x) {sub("DL-moc-0", "", x)})) ## Modify samples names (if required)
